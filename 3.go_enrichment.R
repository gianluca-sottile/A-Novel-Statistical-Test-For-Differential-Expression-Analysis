...
# hy-test
geni.selezionati.our <- unique(names(outPvals05BH))
...
# moderated t-test
geni.selezionati.t <- unique(rownames(topBH))
...
# sam 
geni.selezionati.sam <- unique(names(sam.obj.pv.BH[sam.obj.pv.BH <= .05]))
...
risultati <- subset(risultati, Annotated.hy >= 1 & Annotated.t >= 1 & Annotated.sam >= 1 & Annotated.ebam >= 1)
str(risultati)
summary(risultati)
...
vpj <- list("hy-test" = subset(risult.all, rif.bonf.our == 1)$Term.hy, 
            ...
            "sam" = subset(risult.all, rif.bonf.sam == 1)$Term.sam,
...
venn.diagram(vpj, filename = "VennGO.png",
             imagetype = "png" ,
             height = 640 , 
             width = 640 , 
             resolution = 300,
